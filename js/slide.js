...
(function($) {
    $.fn.slide = function(options) {
        /**
         * 默认设置
         * start: true or false 默认弄1�7姄1�7
         * speed��?0 轮播速度 默认2000 单位：ms
         * animate: "horizontal"||"opacity" horizontal为横向轮撄1�7 opacity为渐变消处1�7
         */
        this.defaults = {
            start: true,
            speed: 5000,
    ...
    function change(showIndex, hideIndex, left) {
        ...
        if (opts.animate == "horizontal") {
            ...
            $(this).find('.slide-container li').eq(hideIndex).stop().fadeOut().animate({
                opacity: 1
            });
            $(this).find('.slide-tabs li').eq(hideIndex).css({
                opacity: 0.7
